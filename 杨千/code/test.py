...
def test_model(feat_model_path, loss_model_path, part, add_loss, device):
    ...
    model = torch.load(feat_model_path, map_location=lambda storage, loc: storage)
    ...
    loss_model = torch.load(loss_model_path,map_location=lambda storage, loc: storage) if add_loss != "softmax" else None
    # if add_loss != "softmax":
    #     loss_model = nn.DataParallel(loss_model, list(range(torch.cuda.device_count())))
    
    
    test_set = ASVspoof2019("LA", "../Features/",
                            "../zju_deepfake/", part,
    ...
    if part == "eval" or part == 'eval2':
        with open(os.path.join(dir_path, 'eval2.txt'), 'w') as cm_score_file:
            for i, (lfcc, audio_fn, tags, labels) in enumerate(tqdm(testDataLoader)):
                ...
                for j in range(labels.size(0)):
                    cm_score_file.write('%s %s\n' % (audio_fn[j],
                                                    "spoof" if score[j].item()>0 else "bonafide"))
                    '''
                    cm_score_file.write(
        ...
        with open(os.path.join(dir_path, 'dev-result.txt'), 'w') as cm_score_file2:
            acc = 0.0;recall = 0.0;pre=0.0
            sum = 0;tp = 0;fn = 0;fp=0;tn=0
            for i, (lfcc, audio_fn, tags, labels) in enumerate(tqdm(testDataLoader)):
                lfcc = lfcc.unsqueeze(1).float().to(device)
                tags = tags.to(device)
                labels = labels.to(device)
                ...
                for j in range(labels.size(0)):
                    sum += 1
                    if(score[j].item()>0):   #spoof
                        if(labels[j].data.cpu().numpy()):
                            tp+=1
                        else:
                            fp+=1
                    else:
                        if(labels[j].data.cpu().numpy()): #bonafide
                            if(labels[j].data.cpu().numpy()):
                                fn+=1
                            else:
                                tn+=1

                    cm_score_file2.write('%s %s\n' % (audio_fn[j],
                                                    "spoof" if score[j].item()>0 else "bonafide"))
...
def test(model_dir, add_loss, device):
    ...
    print("eval2ing...ing...")
    test_model(model_path, loss_model_path, "eval2", add_loss, device)
    #print("deving...ing...")
    #test_model(model_path, loss_model_path, "dev", add_loss, device)


def test_individual_attacks(cm_score_file):
    asv_score_file = os.path.join('../zju_deepfake/',
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--model_dir', type=str, help="path to the trained model", default="./my_best_models/ocsoftmax")
