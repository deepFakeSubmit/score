...
def produce_evaluation_file(dataset, model, device, save_path):
    ...
    for batch_x, utt_id in data_loader:
        ...
        _, batch_pred = batch_out.max(dim=1)
        for tag in batch_pred.tolist():
            if tag == 0:
                label_list.append("spoof")
            else:
                label_list.append("bonafide")
        ...
        with open(save_path, 'a+') as fh:
            for f, cm in zip(fname_list, label_list):
                fh.write('{} {}\n'.format(f, cm))
        fh.close()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='ASVspoof2021 baseline system') # 建立解析对象
    # Dataset 为实例增加属性add_argument
    parser.add_argument('--database_path', type=str, default='../zju_deepfake/',
                        help='Change this to user\'s full directory address of database. We assume that all three train, dev, eval1 data folders are in the same database_path directory.')
    '''
    % database_path/
    %      |- eval2/flac
    %      |- eval1/flac
    ...
    parser.add_argument('--protocols_path', type=str, default='../zju_deepfake/',
                        help='Change with path to user\'s database protocols directory address')
    '''
    % protocols_path/
    %      |- eval2.txt  (eval2数据集无label)
    %      |- eval1.txt  (eval1数据集无label)
    %      |- dev.txt   (dev数据集 & label)
    %      |- train.txt  (train数据集 & label)
    '''
    ...
    parser.add_argument('--model_path', type=str,
                        default='None',
                        help='Model checkpoint. Here provide trained model path to evaluate on Eval databse')
    parser.add_argument('--comment', type=str, default=None,
                        help='Comment to describe the saved model') # 注释来描述保存的模型
    # Auxiliary arguments
    parser.add_argument('--eval_output', type=str, default='None',
                        help='Path to save the evaluation result') # 评估结果保存路径
    parser.add_argument('--eval', action='store_true', default=False,
                        help='eval mode') # train or eval
    ...
    dir_yaml = os.path.splitext('model_config_RawNet')[0] + '.yaml'  # 分离文件名与扩展名

    with open(dir_yaml, 'r') as f_yaml:
        parser1 = yaml.load(f_yaml) #加载模型参数

    if not os.path.exists('models'):
        os.mkdir('models')  # 创建models文件夹
    args = parser.parse_args() #把parser中设置的所有"add_argument"给返回到args子类实例当中， 那么parser中增加的属性内容都会在args实例中，使用即可

    # make experiment reproducible 使实验重现性好
    ...
    # set Adam optimizer 设置优化
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    if args.model_path: # 如果设置了模型路径
    ...
    if args.eval: # 评估
        file_eval = genSpoof_list(dir_meta=os.path.join(
            args.protocols_path + 'eval1.txt'), is_train=False,
                                  is_eval=True)
        print('no. of eval trials', len(file_eval))
        eval_set = Dataset_ASVspoof2021_eval(list_IDs=file_eval,
                                             base_dir=os.path.join(args.database_path + 'eval1/'))
